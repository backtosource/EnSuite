...
class EasyModule(Module):
	...
	def looksForOperation(self, node):
		"""
		Looks if there is any operation attribute in the request
		add by manu 11/9/9
		"""
		
		if node.hasAttributeNS("urn:ietf:params:xml:ns:netconf:base:1.0","operation"):
			return True
		found = False
		for n in node.childNodes:
			if n.nodeType == Node.ELEMENT_NODE:
				found = found or self.looksForOperation(n)
		return found
	...
	def editConfig(self, defaultoperation, testoption, erroroption, target, confignode, targetnode=None):
		...
		** Relates to the netconf edit-config operation
		...
		try:
			...
			# Add by manu 11/9/9
			# if no operation attribute we must explicit the default operation
			if not self.looksForOperation(confignode):
				confignode.setAttributeNS("urn:ietf:params:xml:ns:netconf:base:1.0", "xs:operation", C.MERGE)
			# Add by manu 11/9/9
			...
			print util.convertNodeToString(confignode)
			print "------------------"
			...
			print util.convertNodeToString(generatedStyleSheet)
			print "#################################"
